    ...
    database.observe(DataEventType.value) { snapshot in
      if snapshot.childrenCount > 0 {
        ...
        for product in snapshot.children.allObjects as! [DataSnapshot] {
          ...
          let product = Product(category: productObject?["category"] as! String,
                                price: productObject?["price"] as! String,
                                name: productObject?["name"] as! String,
                                info: productObject?["info"] as! String,
                                date: productObject?["date"] as! String,
                                id: productObject?["id"] as! String
          )
